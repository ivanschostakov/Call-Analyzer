...
export function getWorkspaceShellStyles(
  ...
  return {
    ...
    topbar: {
      ...
      flexWrap: 'wrap',
      alignItems: isMobile ? 'stretch' : compactTopbar ? 'center' : 'flex-start',
      justifyContent: 'space-between',
      gap: compactTopbar ? 10 : 12,
      padding: compactTopbar ? (isMobile ? '10px' : '10px 12px') : compactNav ? (isMobile ? 12 : 16) : 16,
