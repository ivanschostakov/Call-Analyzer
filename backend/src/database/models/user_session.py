...
from config import ufa_now, REFRESH_TOKEN_LIFETIME_DAYS
...
class UserSession(Base, IdPkMixin, TimestampMixin):
    ...
    expires_at: Mapped[datetime] = mapped_column(UfaDateTime(), nullable=False, default=session_expires_at)
    revoked_at: Mapped[datetime | None] = mapped_column(UfaDateTime(), nullable=True, default=None)
    last_used_at: Mapped[datetime | None] = mapped_column(UfaDateTime(), nullable=True, default=None)
